...
"use strict";
  ...
  var storyHeight = $('#content-main').height();
    
      if(storyHeight > 400) {
        $('.read-more-box').show();
        $('#content-main').addClass('read-more-overflow-active');
      } else {
        $('.read-more-box').hide();
      }
    
      $(".read-more-btn").click(function(){
        $('#content-main').removeClass('read-more-overflow-active');
        $('.read-more-box').hide();
      });

  // recomended stories
	function recomended_stories(hits) {
		$.each( hits, function( key, val ) {
		  if (key <= 5) {
			var nstory = val['_source'];
			var nst_card = $('<a class="nst-cards" href="'+nstory['url']+'">');
			nst_card.append("<img src='"+nstory['field_article_images_url']+"' />");
      nst_card.append('<span>'+nstory["title"]+'</span>');

      $('#nst-stories').append(nst_card);
		  } else {
			return false;
      }
